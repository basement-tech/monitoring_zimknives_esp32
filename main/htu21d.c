 ...
 * 
 * June 8, 2024 DJZ:
 * Seems the timing #define/constants may have changed.  
 * Had to do this in the .c
 * <       ret = i2c_master_cmd_begin(_port, cmd, 1000 / portTICK_RATE_MS);
 * ---
 * >       ret = i2c_master_cmd_begin(htu_port, cmd, 1000 / portTICK_PERIOD_MS);
 *
 * Also, when the files are not built as a library, I had to do this in the .h:
 *
 * < i2c_port_t _port;
 * ---
 * > extern i2c_port_t htu_port;
 *
 * And add this to the .c
 * > // global/compartmentalized data
 * > i2c_port_t htu_port;
 */
...
int htu21d_init(i2c_port_t port, int sda_pin, int scl_pin,  gpio_pullup_t sda_internal_pullup,  gpio_pullup_t scl_internal_pullup) {
	...
	conf.master.clk_speed = (uint32_t)1000000;
